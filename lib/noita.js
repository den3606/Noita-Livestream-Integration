...
class Noita {
    ...
    init() {
        ...
        this.youtube.client.on("chat", (chatItem) => {
          console.log("chat in noita.js")
          if (this.voters.indexOf(chatItem.author.name) === -1) {
              this.voters.push(`"${chatItem.author.name}"`)
          }
          this.handleVote(chatItem.author.name, chatItem.message[0].text)
      })
